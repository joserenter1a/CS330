         ...
         int* color, int* distance, int* parent)
{
    ...
    // Initialize variables related to the queue
    adj_node_t **Q = NULL;
    int items_in_Q = 0;

    // Initialize the queue and make sure it's properly allocated
    init_adj_list(&Q, rows);

    // Create a node for the source vertex and add it to the queue
    ...
    while (items_in_Q != 0) {
	// FIFO queue so dequeue first
    	int u = remove_node(Q);
		
	// Decrement the number of items in the queue
	    items_in_Q--;

	// Want to deal with each v in adjacency_list[u] so get starting node v
        adj_node_t* v = list[u];
	
	// Going to go through all of u's neighbors
        while(v != NULL) {
	    // Only add if not already visited or in queue
	        if (color[v->vid] == 0) {

		// Signal that v is now in the queue
	    	color[v->vid] = 1;
		...
		// Create a node to represent v in the queue
		    adj_node_t *v_node = create_node(v->vid);

		// Add the v_node to the queue
		    add_node(Q, 0, v_node);
	...
	// u has been visited and accounted for
	    color[u] = 2;
    }

    // Free the queue that we created
