...
void matrix_transpose(int** dst, int** src, int rows, int cols)
{
	...
	// Loop through all the columns in the matrix
    	for (int j = 0; j < cols; j++) {
	    // Create a temp variable to store the desired value
            int temp = src[i][j];
            // Store the value in the correct location in the transpose matrix
            dst[j][i] = temp;
	}
    }
}
              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        int neighbor_i = 0;
        int i, j;
        // make dst vector all zeroes
        reset_vector(dst, cols);

        // given a vector of source vertices, find its neighbors
        for (i = 0; i < cols; i++) {
            if (src[i] == 1) {
                for (int j = 0; j < rows; j++) {
                // Determine if src has a neighbor
        ...
        // set the src vertices color to 2
        for (i = 0; i < rows; i++) {
            if (dst[i] == 1)
            color[i] = 2;
        }
        // store distances for neighbors
        for (i = 0; i < rows; i++) {
            if (dst[i] == 1){
                distance[i] = iter; 
            }
        ...
        // if no neighboring indices
        if (!neighbor_i){
            done = 1; //done
        }
        iter++; // increment breadth
    }
