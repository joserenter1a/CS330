                        ...
                        double** csr_vals)
{
    ...
    unsigned int* bins = malloc(sizeof(unsigned int) * m);
    memset(bins, 0, sizeof(unsigned int) * m);
          ...
          double* vector_x, double* res)
{
    ...
    unsigned int i, j;
    unsigned int row_start, row_fin;
    for(int i = 0; i < m; i++) {
        res[i] = 0.0;
    }
    ...
    for(i = 0; i < m; i++) {
        ...
        row_fin = csr_row_ptr[i + 1];
        for(j = row_start; j < row_fin; j++) {
...
int main(int argc, char** argv)
{
    ...
    read_vector(vectorName, &vector_x, (int*) &vector_size);
